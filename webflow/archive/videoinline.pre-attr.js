 ...
 */
(function () {
  // Both naming conventions, in one selector each.
  const ICON = {
    play: '.videoinline_play_icon, .card_play_icon',
    pause: '.videoinline_pause_icon, .card_pause_icon',
    volumeUp: '.videoinline_volume_up_icon, .card_volume_up_icon',
    volumeMute: '.videoinline_volume_mute_icon, .card_volume_mute_icon',
    maximise: '.videoinline_fullscreen_maximise, .card_fullscreen_maximise',
    minimise: '.videoinline_fullscreen_minimise, .card_fullscreen_minimise'
  };
  ...
  const init = () => {
    ...
    document.querySelectorAll('[data-videoinline-player]').forEach((player) => {
      ...
      const posterSrc = () =>
        ...
        buildSources(player.getAttribute('data-video-id')).poster;
      ...
      const loadPreview = () => {
        ...
        const src =
          ...
          buildSources(player.getAttribute('data-video-id')).preview;
        if (!src) return;
        previewLoaded = true;
        preview.src = src;
      };
