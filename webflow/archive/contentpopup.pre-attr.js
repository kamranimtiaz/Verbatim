...
${old.textContent}
  ...
  document.querySelectorAll(".popup_wrap").forEach((component) => {
    ...
    // --- Openers -----------------------------------------------------------
    const bindOpeners = () => {
      // .r_articles_row_wrap covers "Related articles" rows inside injected
      // content — clicking one swaps the popup's content instead of navigating.
      document
        .querySelectorAll(".card_read, [data-popup-open], .r_articles_row_wrap")
        .forEach((opener) => {
        ...
        opener.addEventListener("click", (e) => {
          ...
          e.preventDefault();
          const card = opener.closest("[data-card], .card_wrap") || opener;
          const title =
            opener.getAttribute("data-popup-title") ||
            card.querySelector("[data-card-title], .card_title")?.textContent.trim();
